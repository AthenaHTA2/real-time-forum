...
async function chatEventHandler() {
  ...
  console.log("the js object of messages +++++++++++>:",messages)
  //console.log("the reversed js object of messages +++++++++++>:",messages.reverse())
    ...
    item.onclick = () => {
      ...
      messages.forEach((message) => {
        ...
        if (message.sender === CurrentUser && message.recipient === receiver) {
            //store message in the 'MessagesForDisplay' array
            MessagesForDisplay.push(message)
          
          
        } else if (
          // 2. when current user is receiver
  ...
  console.log("Chat messages history:=====>",MessagesForDisplay)
  ...
  chatScroll.addEventListener("scroll" ,function(evt){
    if(chatScroll.scrollTop == 0){
      ...
      MsgsInChat = ParentDiv.children.length -1 - CountNewMessages;//<=== subtracting 1 to get correct #of messages
...
function addTen (messages, limit){
          ...
          //print available messages in chunks of 10 or less
            for(let m = arrayPosition; m > (arrayPosition - limit); m--){
              ...
              console.log("printing the message details:+++",messages[m])
                if (messages[m].sender === CurrentUser && messages[m].recipient === receiver){ 
                ...
                else if (messages[m].recipient === CurrentUser && messages[m].sender === receiver) {
  ...
  if (window["WebSocket"]) {
    ...
    conn.onclose = function (evt) {
      ...
      //appendLog(item);//appendLog has been replaced by functions: a)appendUser; b)addTen
    ParentDiv.append(item);
    };
    //here we instantaneously print new messages
