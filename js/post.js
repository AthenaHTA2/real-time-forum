...
//let category_1 = document.querySelector('input[name="postType"]:checked');;
  // Get category
//let category_2 = document.getElementById("select-category");
  //grab the post data
    let theCookie = GetCookie("user_session"); //get the session cookie
    postCookie = theCookie  //store the session cookie in postCookie variable
    console.log("++++_+_+_+_+_+_+_+_====================",theCookie)
    let PostTitle = document.querySelector("#PostTitle").value
    let PostContent = document.querySelector("#PostContent").value
    let PostCategory = document.querySelector("#cat").value
...
const successfulPost = () => {
    ...
    PostTitle.value = ""
    PostContent.value = "" //clear the content fiel
    document.querySelector("#PostCat").value = ""
  //document.querySelector("#cars").value = "Weather"   
  //document.querySelector("#select-category").value = "" //clear the category field
};
...
//convert jS time stamp into a string, using when displaying posts
const ConvertDate = (date) => {
  // Seperate year, day, hour and minutes into vars
  let yyyy = date.slice(0, 4);
  let dd = date.slice(8, 10);
